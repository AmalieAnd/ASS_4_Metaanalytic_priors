...
library(brms)
library(tidyverse)
pacman::p_load(readxl, coda, bayesplot)
...
```{r real_updated_cons_model}
pitch_cons_2 <- brm(
  pitchSD_s ~ 1 + diagnosis + (1 | studynr) + (1 | ID_unique) + (1 | trial), 
  prior = c(set_prior("normal(0, 0.1)", class = "Intercept"), 
            set_prior("normal(0, 0.1)", class = "b", coef = "diagnosis"),
            set_prior("cauchy(0, 0.1)", class = "sd", coef = "Intercept", group = "studynr"),
            set_prior("cauchy(0, 0.1)", class = "sd", coef = "Intercept", group = "ID_unique"), 
            set_prior("cauchy(0, 0.1)", class = "sd", coef = "Intercept", group = "trial"),
            set_prior("cauchy(0, 1)", class = "sigma")),
  data = pitch_s
)
...
pitch_meta <- brm(
  pitchSD_s ~ 1 + diagnosis + (1 | studynr), 
  prior = c(set_prior("normal(0, 0.1)", class = "Intercept"),
            set_prior("normal(-0.61 , 0.3)", class = "b"), 
...
```{r compare models}
compare(pitch_cons, pitch_meta)

loo_compare(pitch_cons, pitch_meta)
